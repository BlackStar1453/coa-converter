...
> ⚠️ **强制要求**：此步骤是**必须执行**的，不可跳过。Python 转换脚本（Step 3）仅替换产品名，**不会自动填充营养数据**——输出文件中营养数据表的所有数值均为**模板占位符**。必须由 Claude 通过以下网络搜索步骤手动获取真实营养数据并更新输出文件，才可进入 Step 4 验证。**严禁将模板占位符值直接保留在输出文件中。**
